 ...
 * 在设置页注册侧边栏入口「任务通知 (dsh-helper)」，表单保存重试次数
 * 与各通知开关；并提供「发送测试通知」按钮验证通知链路。
...
function TaskNotifierSettings() {
  ...
  const doSave = async () => {
    ...
    try {
      const value = {
        retryMax: Number.isInteger(draft.retryMax) ? draft.retryMax : 3,
        retryBaseMs: Number.isInteger(draft.retryBaseMs) ? draft.retryBaseMs : 1000,
  ...
  return h("div", { className: "dshh-settings" }, [
    h("header", { className: "dshh-header" }, [
      h("span", { className: "dshh-kicker" }, "dsh-helper · 任务通知 / 自动重试"),
      h("h2", null, "任务通知"),
      h("p", null, "agent 会话任务完成 / 异常 / 需要确认时弹 Windows 通知；请求异常（超时、断连、限流等）按下方次数自动重试。"),
    ]),
    h("section", { className: "dshh-panel" }, [
      h("div", { className: "dshh-grid" }, [
        h("div", { className: "dshh-field" }, [
          h("label", null, "自动重试次数（0 = 关闭）"),
          numInput(draft.retryMax, (v) => update({ retryMax: v }), "3"),
        ]),
        h("div", { className: "dshh-field" }, [
          h("label", null, "重试基础退避 (ms，指数增长，上限 30s)"),
          numInput(draft.retryBaseMs, (v) => update({ retryBaseMs: v }), "1000"),
        ]),
      ]),
