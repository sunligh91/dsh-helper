...
/**
 * WinRT Toast 的 AUMID。用 Windows 始终注册过的 File Explorer AUMID，
 * 避免自定义 AUMID('dsh-helper') 在非打包应用里被 Windows 静默丢弃
 * （Show() 不报错但通知永不显示）。这是确认通知「已调用却看不到」的根因。
 */
const TOAST_APP_ID = '{1AC14E77-02E7-4E5D-B744-2EB1AE5198B7}\\explorer.exe'
...
$t = $tn.GetTemplateContent([Windows.UI.Notifications.ToastTemplateType]::ToastText02)
