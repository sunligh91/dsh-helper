 ...
 *
 * 功能：
 *  1. agent 会话任务完成 / 异常 / 需要确认时，弹 Windows 通知（PowerShell
 *     WinRT Toast，零外部依赖；如需更可靠可后续引入 node-notifier）。
 *  2. agent 请求异常（失败/超时/瞬态）时按设置次数自动重试：在
 *     `agent/request-error` waterfall 中返回 { kind: 'retry' } 让 dsh 重发，
 *     并维护按会话计数的重试预算 + 指数退避。
 *
...
const RETRYABLE_HINTS = [
  ...
  'ENOTFOUND', 'TRANSPORT', 'RATE_LIMIT', 'RATE_LIMITED',
]
...
function mergeConfig(input) {
  ...
  out.retryMax = Number.isInteger(out.retryMax) && out.retryMax >= 0 ? out.retryMax : DEFAULTS.retryMax
  out.retryBaseMs = Number.isInteger(out.retryBaseMs) && out.retryBaseMs >= 0 ? out.retryBaseMs : DEFAULTS.retryBaseMs
...
function isRetryable(code, failure) {
  const c = typeof code === 'string' ? code.toUpperCase() : ''
  for (const h of RETRYABLE_HINTS) {
    if (c.includes(h)) return true
  }
  const msg = typeof (failure && failure.message) === 'string' ? failure.message : ''
  return /timeout|econnreset|econnrefused|enotfound|network|rate.?limit|429|throttl|5\d\d server/i.test(msg)
}
...
export function apply(ctx, config = {}) {
  ...
  // 2. 异常通知 + 自动重试（agent 请求错误）
  ctx.on('agent/request-error', async (payload, next) => {
    const agent = payload && payload.agent
    const sid = agent && agent.id
    const failure = payload && payload.failure
    const code = failure && typeof failure.code === 'string' ? failure.code : ''
    const retryable = isRetryable(code, failure)

    if (options.notifyOnError) {
      notify('⚠️ 任务异常', `会话 ${shortId(sid)} 出错${code ? ` (${code})` : ''}`)
    }

    if (retryable && options.retryMax > 0) {
      const now = Date.now()
      let bucket = shared.retries.get(sid)
      if (!bucket || now - bucket.windowStart > 60000) {
        bucket = { windowStart: now, count: 0 }
        shared.retries.set(sid, bucket)
      }
      if (bucket.count < options.retryMax) {
        bucket.count += 1
        const delay = Math.min(options.retryBaseMs * 2 ** (bucket.count - 1), 30000)
        notify('🔄 正在重试', `会话 ${shortId(sid)} 第 ${bucket.count}/${options.retryMax} 次（约 ${Math.round(delay / 1000)}s 后）`)
        await sleep(delay)
        return { kind: 'retry' }
      }
      notify('⛔ 重试已达上限', `会话 ${shortId(sid)} 已重试 ${options.retryMax} 次，停止`)
    }
    return next()
  })
