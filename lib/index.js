...
export function apply(ctx, config = {}) {
  ...
  ctx.on('agent/status', ({ agent, status }) => {
    ...
    // 全局节流：60 秒内最多一条完成通知（多主会话同时收尾也只弹一条）
    if (now - shared.lastCompleteAny < 60000) {
      debugLog(`global throttle: suppress complete notify for ${shortId(sid)}`)
      return
    }
    shared.lastCompleteAny = now
